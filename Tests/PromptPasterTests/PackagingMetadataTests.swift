...
final class PackagingMetadataTests: XCTestCase {
    ...
    func testReleaseWorkflowBuildsValidatesAndPublishesDMG() throws {
        ...
        XCTAssertTrue(workflow.contains("Align release tag to validated commit"))
        XCTAssertTrue(workflow.contains("git tag --force \"$TAG_NAME\" \"$GITHUB_SHA\""))
        XCTAssertTrue(workflow.contains("git push --force origin \"refs/tags/$TAG_NAME\""))
